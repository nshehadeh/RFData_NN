...
clear
load('chandat1.mat', 't', 'fs', 'c', 'angles', 'apex', 'env')
load('new_data1.mat')
...
% Compute SNR-- this is probably the most important metric to consider
% Note that this requires multiple frames to measure, hence the need to
% simulate multiple frames with different noise realizations. The bigger
% this is the better

%datpath = 'testing_data/20211216/input/phantom_1/';
numFrames = 4;
load('chandat1.mat')
load('new_data1.mat')
env_total = zeros(size(rf_data, 1), size(rf_data, 2), numFrames);
env = permute(new_env, [2, 1]);
env_total(:, :, 1) = env;
for nn = 2:numFrames
    load(fullfile(['new_data', num2str(nn), '.mat']))
    env = permute(new_env, [2, 1]);
    env_total(:, :, nn) = env;
end

W = round(10*fs/f0); % 10 wavelength kernel
Rt = calculateBeamformedTemporalLOC(env_total, W); 
SNR = Rt./(1-Rt);
SNR = 10*log10(mean(SNR(:))) % SNR in dB

% Compute dynamic range-- bigger is better. This is probably this least
% important image quality metric to take into account
DNR = max(20*log10(env_total(:))) - min(20*log10(env_total(:))) % dB
